...
class InferenceHandler:
    ...
    def __init__(self, weight_path, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')) -> None:
        ...
        model.load_state_dict(torch.load(
            weight_path, map_location='cpu', weights_only=False), strict=True)
        model.eval()
    ...
    # 修改后：
    def inference(self, audio_path, outpath=None, text_edit=None, valid_programs=None, num_beams=4):
